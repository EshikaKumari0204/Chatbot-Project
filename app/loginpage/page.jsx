...
return (
  <div className="min-h-screen bg-slate-900 flex flex-col text-amber-50 p-10 justify-center items-center">
    ...
    <form onSubmit={handlesubmit} className="flex flex-col text-2xl gap-2 justify-center items-center" >
      ...
      <button type="submit" className="border-1 px-2 py-1 rounded-2xl bg-gray-400 w-fit text-[12px] md:text-[18px]">{signup?"Sign Up":"Login"}</button>
      <p className="text-[16px] md:text-2xl text-center">{signup?"Login to your account  ":"Don't have an account? "}<br/><button type="button" className="border-1 px-2 py-1   rounded-2xl bg-gray-400 w-fit text-[12px] md:text-[18px] mt-3" onClick={()=>{setsignup(!signup);setmess("");setusername("");setpass("");}}>{signup?"Login":"Sign Up"}</button></p>
   {loading && <p className="mt-3 text-amber-200 text-center text-[16px] md:text-[22px]">{signup? "Signing Up !! wait  ":"Logging in !! wait"}</p>}
